...
func (immudbRequester ImmudbRequester) confirmBuyer(user_id string, signature string, pufID string) bool {
	...
	dbLockupTime := time.Since(start)
	start = time.Now()
	log.Printf("DBLOOKUP VERIFICATION: %s", dbLockupTime)
	...
	h.Write([]byte(pufID))
	d := h.Sum(nil)
	verification := rsa.VerifyPSS(pk, crypto.SHA256, d, sigBytes, nil)
	verificationTime := time.Since(start)
	log.Printf("SIGNATURE VERIFICATION: %s", verificationTime)
...
func (immudbRequester ImmudbRequester) getDatabaseType() string {
	return "immudb"
}
